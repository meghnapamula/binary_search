    ...
    def go(xs, left, right):
        ...
        if xs[mid] > 0:
            ...
            if xs[mid - 1] <= 0:
                ...
                return go(xs, left, mid - 1)
        else:
            return go(xs, mid + 1, right)
...
def count_repeats(xs, x):
    ...
    Use the following three step procedure:
        1) use binary search to find the lowest index
        with a value >= x
        2) use binary search to find the lowest index
        with a value < x
        3) return the difference between step 1 and 2
    I highly recommend creating stand-alone functions
    for steps 1 and 2,
    ...
    a = lowest(xs, x, 0, len(xs) - 1)
    if a == -1:
        return 0
    b = highest(xs, x, a, len(xs) - 1)
    return b - a + 1
...
def argmin(f, lo, hi, epsilon=1e-3):
    ...
    a float as input and returns a float with a
    ...
    different f functions that can be minimized and
    their applications.
